...
export function testparam() {
    ...
    if (localStorage.getItem('userInfo') && JSON.parse(localStorage.getItem('userInfo')).userToken) {
        userToken = JSON.parse(localStorage.getItem('userInfo')).userToken
    }
    let os = detectOS()//操作系统版本
    let app = ''
    process.env.type === 'guided' || process.env.type === 'guidet' ? app = 'm-c' : app = 'm-u'
    ...
    if (getCookie('sc') && getCookie('sc') != 'null') {
...
/**
 * @description 根据机型选择时间日期item间的高度
 * @return {number}
 */
 ...
 * @params fun delay
 */
export function debounceDelay(fun, delay = 300) {
    if (fun.timer) {
